    ...
    <View style={{width: "100%", alignItems: "center", justifyContent: "center"}}>
      ...
      {holidays.length ? (
        holidays.map((holiday) => {
          return (
            <View style={{ width: "100%", borderBottomWidth: 2, borderBottomColor: "gray", margin: 20}}>
              <View style={{display: "flex", alignItems: "center"}}>
                <Text style={{ fontSize: 25, fontWeight: 700, marginBottom: 10}}>{holiday.name}</Text>
                <Text style={{ color: "red", fontSize: 20, marginBottom: 20}}>{holiday.date}</Text>
              </View>
            </View>
          );
        })
